...
export class AppComponent implements OnInit {
  ...
  public onAddEmloyee(addForm: NgForm): void {
    const closeBtn = document.getElementById('add-employee-form');
  ...
  public onDeleteEmloyee(): void {
    if (this.deleteEmployee && this.deleteEmployee.id !== undefined) {
      ...
      console.error('Cannot delete employee: Invalid ID');
    }
  }
  ...
  public onOpenModal(employee: Employee | null, mode: string): void {
    ...
    if (mode === 'edit' && employee !== null) {
      this.editEmployee = employee; // Assign only if employee is not null
      button.setAttribute('data-target', '#updateEmployeeModal');
    }
    if (mode === 'delete' && employee !== null) {
      this.deleteEmployee = employee; // Assign only if employee is not null
